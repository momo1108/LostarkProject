- 캐릭터 조회 성향 자리에 각인으로 대체하고 각인 자리에 아크패시브로 대체
- EquipmentSlot, AccessorySlot 둘다 유틸 함수로부터 파싱하도록 변경(나중에 리팩토링할것)
...
- Web Worker 를 React 용으로 패키지 개발?
  - 객체 형태로 return?
  - worker 에 메세지 모내는 것을 동기처리해주는것까지 구현된 메서드, status, 원본 참조 등...
...
자세한 검색조건은 apirequest_auction, apirequest_market.json 을 참조.
...
- 툴팁 위치를 옮기는 대작업을 해야한다. O
- 특성/성향 모달로 출력기능(직접 출력으로 수정)
- 아이디내 캐릭터 보기(API내에 제공)
- 순위는 어떻게 했을까?(직접 검색한 정보를 DB에 갱신하는듯. 무료서버로는 힘들것 같다.)
- 차트라이브러리 혹시라도 참조(charjs - https://react-chartjs-2.js.org/)
- 각인 CSS
  - 배경찾기
- 각인 검색 보유악세
- 각인 검색 유물등급(섞는 경우?) O - (나중에) 매물이 모자라서 검색결과가 안나오면 추가검색하게 하는 기능? 섞는 경우 고대, 유물 따로 검색?(매물이 적어서...)
- 각인 검색 같은 특성 O
...
- apiKey 발급 안내페이지 O
- api검색도 webworker로 분리하기 O
- api검색 or 필터링 취소? -> 취소 대신 비활성화로 대체 O
- seo - 패키지사용하자.
...
- 각인 검색 목록 비었을 때 추가.
- 방문 유저 수 체크하기.(쿠키 사용하면 될듯)
- 직업 BG 이미지 추가
- 나머지 트포작업
  - 어드민 페이지에서 해야할 것
    - 트포 검색 관련 닉네임 수정기능
    - 검색을 통해 스킬 정보를 갱신 후 저장
  - 지금 클래스별 스킬셋 데이터 용량이 너무 크다. 분리방법을 강구해야함. (O)
    - 클래스별로 스킬이름만 따로
    - 스킬 클릭하면 트포 불러오기?
  - admin 페이지에서 사용할 hook 기능에 갱신 데이터 저장도 필요하다.(DB가 나을라나?)
    - 아마도 Next의 api기능을 활용해서 admin page -> 서버에 요청보내 저장해야 할듯.
  - 선택된 스킬의 인덱스를 사용하니 나중에 스킬 추가했을 때, 그 새로운 배열의 인덱스에 적용이 되서 이상해짐. (O)
    - 추가의 경우 선택된 스킬 이전의 스킬이면 인덱스 + 1
    - 삭제의 경우 선택된 스킬이 0보다 클때 작은 스킬은 -1 같은 스킬은 유지(마지막 스킬인 경우 -1), 큰 스킬은 유지
  - 검색기능
  - 초기화기능
  - 캐릭터명으로 불러오기기능
...
- 복사 성공 시 성공 alert
- 트포 페이지에선 성공 알러트 사운드가 제대로 나옴. 트포 페이지를 거치지 않고 바로 각인 페이지 접속 시 성공 사운드(인지 아니면 modal문제인지)가 null로 나옴
...
- 캐릭터 검색 미리보기 이미지 링크
- firefox 각인서택 팝업창 스크롤바 너비(기본 스크롤바 css 삭제로 해결.)
...
- 새 아바타, 장비 슬롯이 추가되었을 때
  ...
  - 새 슬롯 관련 빈칸 이미지 추가
- 트포 구현 관련 문제
  - 스킬셋을 가져오려면, 현재 api로는 캐릭터 검색을 통한 방법뿐이다. 따라서 구현 관련 방법은 2가지를 생각해보았다.
  - 1. 클래스별 닉네임을 고정해놓고 사용자가 클래스 선택 시 실시간으로 스킬셋을 api로 받아온다.
    - 장점) 실시간 데이터이므로, 스킬셋에 업데이트가 있어도 최신 정보를 받아온다.
    - 단점) 검색에 필요한 닉네임이 유효하지 않으면 데이터를 불러오지 못한다.
  - 2. 클래스별 스킬셋을 서버쪽에 저장해놓고, 사용자가 클래스 선택 시 서버에 저장된 정보를 준다.
    - 장점) 닉네임의 유효여부와 상관없이 데이터를 가져올 수 있다.
